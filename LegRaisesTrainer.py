...
def run():
    ...
    #Angle thresholds for leg raises (shoulder-hip-knee)
    FULL_DOWN_ANGLE = 150  
    FULL_UP_ANGLE = 120    
    ...
    leg_status_right, leg_color_right = "STRAIGHT", GREEN
    leg_status_left, leg_color_left = "STRAIGHT", GREEN
    ...
    while True:
        ...
        if landmarks_available(lmList, right_leg_points):
            ...
            if duration_visible > VISIBILITY_THRESHOLD:
                ...
                percentage_right = np.interp(angle_right, (FULL_UP_ANGLE, FULL_DOWN_ANGLE), (100, 0))
                
                #Form Check
                straight_leg_points = [24, 26, 28] 
                if landmarks_available(lmList, straight_leg_points):
                    leg_angle = _norm_angle(detector.findAngle(img, *straight_leg_points, draw=False))
                    if leg_angle >= STRAIGHT_LEG_ANGLE_MIN: leg_status_right, leg_color_right = "STRAIGHT", GREEN
                    else: leg_status_right, leg_color_right = "BENT KNEE", RED
                
                if percentage_right >= 95 and dir_right == 0 and leg_status_right == "STRAIGHT": count, dir_right = count + 0.5, 1
                if percentage_right <= 5 and dir_right == 1: count, dir_right = count + 0.5, 0
                
                bar_height_right = np.interp(percentage_right, (0, 100), (BAR_MAX_Y, BAR_MIN_Y))
        ...
        if landmarks_available(lmList, left_leg_points):
            ...
            if duration_visible > VISIBILITY_THRESHOLD:
                ...
                percentage_left = np.interp(angle_left, (FULL_UP_ANGLE, FULL_DOWN_ANGLE), (100, 0))

                straight_leg_points = [23, 25, 27]
                if landmarks_available(lmList, straight_leg_points):
                    leg_angle = _norm_angle(detector.findAngle(img, *straight_leg_points, draw=False))
                    if leg_angle >= STRAIGHT_LEG_ANGLE_MIN: leg_status_left, leg_color_left = "STRAIGHT", GREEN
                    else: leg_status_left, leg_color_left = "BENT KNEE", RED

                if percentage_left >= 95 and dir_left == 0 and leg_status_left == "STRAIGHT": count, dir_left = count + 0.5, 1
                if percentage_left <= 5 and dir_left == 1: count, dir_left = count + 0.5, 0

                bar_height_left = np.interp(percentage_left, (0, 100), (BAR_MAX_Y, BAR_MIN_Y))
        ...
        cv.putText(img, leg_status_right, (60, BAR_MAX_Y + 40), cv.FONT_HERSHEY_PLAIN, 2, leg_color_right, 2)
        ...
        cv.putText(img, leg_status_left, (1050, BAR_MAX_Y + 40), cv.FONT_HERSHEY_PLAIN, 2, leg_color_left, 2)
        ...
        cv.imshow('GymGuardian - Leg Raises', img)
